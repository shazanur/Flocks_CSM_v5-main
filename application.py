...
from utils.risk_scorer import score_dataframe          # ← composite risk scoring
...
# ══════════════════════════════════════════════════════════════════════════════
#  AI INCIDENT CLASSIFIER — constants, model class & render helpers
# ══════════════════════════════════════════════════════════════════════════════
CLF_C = {
    'card':    '#1A1D27',
    'accent':  '#7C3AED',
...
_LABEL_MAP = {
    'Malware': 'Intrusion System', 'Ransomware': 'Intrusion System',
    'Ransomware Attack': 'Intrusion System', 'Advanced Persistent Threat (APT)': 'Intrusion System',
    'APT': 'Intrusion System', 'DDoS': 'Intrusion System',
    'Vulnerability': 'Intrusion System', 'Zero-Day': 'Intrusion System',
    'Supply Chain': 'Intrusion System', 'Supply Chain Attack': 'Intrusion System',
    'Unauthorised Access': 'Intrusion System', 'Exploit': 'Intrusion System',
    'Botnet': 'Intrusion System', 'Botnet control': 'Intrusion System',
    'Attack': 'Intrusion System', 'Cyber Attack': 'Intrusion System',
    'Cyberattack': 'Intrusion System', 'Hack': 'Intrusion System',
    'Hacking': 'Intrusion System', 'Intrusion': 'Intrusion System',
    'Handala': 'Intrusion System',
    'Data Breach': 'Data Breach', 'Insider Threat': 'Data Breach',
    'Data Leak': 'Data Breach', 'Sell Data': 'Data Breach',
    'Data Breached': 'Data Breach', 'Breach': 'Data Breach',
    'Phishing': 'Compromise of Credentials', 'Social Engineering': 'Compromise of Credentials',
    'Credential Stuffing': 'Compromise of Credentials', 'Account Takeover': 'Compromise of Credentials',
    'Brute Force': 'Compromise of Credentials', 'Compromise Credentials': 'Compromise of Credentials',
    'Fraud': 'Scam/Fraud', 'Financial Fraud': 'Scam/Fraud',
    'Scam/Fraud': 'Scam/Fraud', 'Advertising fraud': 'Scam/Fraud', 'Scam': 'Scam/Fraud',
    'Cybersecurity': 'Other', 'Other Cyber Incident': 'Other Cyber Incident',
    'Others': 'Other', 'Other': 'Other', 'cybersecurity incident': 'Other',
}
...
class IncidentClassifier:
    ...
    def train(self, df):
        ...
        if df.empty:
            return False
        df['incident_type'] = df['incident_type'].replace(_LABEL_MAP)
    ...
    def predict_one(self, text):
        ...
        attack_keywords = [
            'attack', 'hack', 'malware', 'ransomware', 'ddos',
        ...
        return {'label': label, 'confidence': proba.max(), 'top3': top3}


# ── Classifier render helpers ─────────────────────────────────────────────────

def _clf_kpi(col, title, value, sub='', colour='#7C3AED'):
    with col:
        st.markdown(
            f"""<div style="background:{CLF_C['card']};border-left:4px solid {colour};
                padding:16px 20px;border-radius:8px;margin-bottom:4px;">
                <div style="font-size:11px;color:{CLF_C['subtext']};text-transform:uppercase;
                            letter-spacing:.08em;">{title}</div>
                <div style="font-size:28px;font-weight:700;color:{CLF_C['text']};">{value}</div>
                <div style="font-size:12px;color:{CLF_C['subtext']};">{sub}</div>
            </div>""",
            unsafe_allow_html=True,
        )


def _clf_section(title, icon=''):
...
def _metric_colour(val: float, thresholds=(0.70, 0.50)) -> str:
    if val >= thresholds[0]: return CLF_C['green']
    if val >= thresholds[1]: return CLF_C['orange']
    return CLF_C['red']
...
def _render_eval_card(clf: IncidentClassifier):
    ...
    if not m:
        return
    _clf_section('Model Evaluation Card', '📊')
    c1, c2, c3, c4, c5 = st.columns(5)
    _clf_kpi(c1, 'Accuracy',        f"{m['accuracy']*100:.1f}%",        'Overall correctness', _metric_colour(m['accuracy']))
    _clf_kpi(c2, 'Precision (wtd)', f"{m['precision_weighted']:.4f}",   'Weighted avg',        _metric_colour(m['precision_weighted']))
    _clf_kpi(c3, 'Recall (wtd)',    f"{m['recall_weighted']:.4f}",      'Weighted avg',        _metric_colour(m['recall_weighted']))
    _clf_kpi(c4, 'F1 Score (wtd)',  f"{m['f1_weighted']:.4f}",          'Weighted avg',        _metric_colour(m['f1_weighted']))
    _clf_kpi(c5, 'F1 Score (macro)',f"{m['f1_macro']:.4f}",             'Unweighted avg',      _metric_colour(m['f1_macro']))


def _render_training_curve(clf: IncidentClassifier):
    _clf_section('Training Loss Curve', '📉')
    loss = clf.loss_curve
    if not loss:
    ...
    fig.add_trace(go.Scatter(
        ...
        name='Training Loss', line=dict(color=CLF_C['accent'], width=2),
...
def _render_confusion_matrix(clf: IncidentClassifier):
    ...
    fig = px.imshow(
        ...
        text_auto=True, title='Confusion Matrix — Actual vs Predicted', aspect='auto',
    )
    fig.update_layout(
...
def _render_per_class(clf: IncidentClassifier):
    _clf_section('Per-Class Performance Report', '📋')
...
def _render_live_classifier(clf: IncidentClassifier):
    _clf_section('Live Incident Classifier', '🔍')
    st.markdown(
    ...
    text_input = st.text_area(
        'Incident headline / summary', height=100,
    ...
    if st.button('🔮 Classify Incident', use_container_width=True, type='primary'):
        ...
        with st.spinner('Analyzing...'):
            ...
            top3   = result['top3']
            colour = CLF_SEVERITY_COLOUR.get(label, CLF_C['accent'])
            st.markdown(
                f"""<div style="background:{CLF_C['card']};border-left:6px solid {colour};
                    padding:20px 24px;border-radius:10px;margin:16px 0;">
                    <div style="font-size:12px;color:{CLF_C['subtext']};text-transform:uppercase;
                    ...
                    <div style="font-size:14px;color:{CLF_C['subtext']};">
...
# ══════════════════════════════════════════════════════════════════════════════
#  PAGE 4 — AI INCIDENT CLASSIFIER  (MLP Neural Network · TF-IDF)
# ══════════════════════════════════════════════════════════════════════════════
def page_ai_classifier():
    ...
    page_header(
        ...
        "MLP Neural Network · TF-IDF Text Features · Aligned to Malaysia NAIO 2026–2030",
    )
    ...
    with st.spinner("Loading cyber news data…"):
        df = load_cyber_news()

    if df is None or df.empty:
        st.warning("No data available in the global_news table.")
        return

    # Ensure required columns exist for the classifier
    ...
    if 'summary' not in df.columns:
        for alt in ('description', 'content', 'body'):
            if alt in df.columns:
                df['summary'] = df[alt]
                break
        else:
            df['summary'] = ''
    if 'incident_type' not in df.columns:
        for alt in ('type', 'attack_type', 'threat_type', 'category'):
            if alt in df.columns:
                df['incident_type'] = df[alt]
                break
        else:
            st.error("⚠️ cyber_news table has no incident_type / category column.")
            return

    cache_key = f'clf_{len(df)}'

    if retrain or cache_key not in st.session_state:
        with st.spinner('Training MLP classifier on cyber news data…'):
            clf = IncidentClassifier()
            if clf.train(df):
                st.session_state[cache_key] = clf
                st.success(f'✅ Model trained on {len(df):,} records.')
            else:
                st.error('Model training failed — check that cyber_news has enough labelled rows.')
                return
    else:
